...
char *readFromUserFile(char *name, int *length) {
    ...
    *length = pos.__pos;
    ...
    char *content = static_cast<char*>(malloc(sizeof(char)* (*length)));
...
void createEmptyVirtualDisk() {
    ...
    for(int i=0; i<64; ++i) fwrite(encryptor, sizeof(char), 20, disk); //deskryptory_plikow = 1280B
    //razem rozmiar: 4261121B
    fclose(disk);
}

int addFileToVirtualDisk(char *content, int length, char *fileName) { //return: 2-error open disk, 3-disk is full, 1-fragmetnacja zewnetrzna-nie ma miejsca, 0-udalo sie dodac
...
int removeFileFromVirtualDisk(char *fileName) { //fileName to char[15] ///TODO: dopisac zmiane bitmapy
    FILE *disk = fopen("MyVirtualDisc", "r+b");
    if(disk==NULL) {
        printf("Nie udalo sie otworzyc dysku!\n");
        return 2;
...
char * readFileFromVirtualDisk(char *searchName, int *fileSize) { //przekazana nazwa ma byc char[15]; do wskaznika content zostanie wstwiony plik, do size bedzie wstawiony rozmiar pliku
    ...
    *fileSize=0;
    ...
    for(int i=0; i<64; ++i) {
        ...
        if((int)flags==1 && strcmp(searchName, name)==0) { //znalazlem enkrytptor danego pliku
            ...
            content = static_cast<char*>(malloc(sizeof(char)*size)); //alokacja pamieci na dane
...
void printFileFromVirtualDisk(char *fileName){
    FILE *disk = fopen("MyVirtualDisc", "rb");
    if(disk==NULL) {
        printf("Nie udalo sie otworzyc dysku!\n");
        return;
...
void comCopyFileToDisk(){
    ...
    int result = addFileToVirtualDisk(content, length, name);
    free(content);
    if(result == 0)
        printf("Dodano plik\n");
}
void comCopyFileFromDisk(){
    ...
    char *content;
    int fileSize;
    content = readFileFromVirtualDisk(name, &fileSize);
    if(content==NULL){
        printf("ERROR\n");
        return;
    }
...
void comRemoveFile(){
    ...
    scanf("%s", name);
    clearEndLine(name);
    if(removeFileFromVirtualDisk(name)==0)
...
int main(int argc, char *argv[]) {
    if(argc>=2){
        ...
        else if(option==4) printFilesInVirtualDisk();
    ...
    while(whatDo){
        ...
        else if(whatDo==4) printFilesInVirtualDisk();
